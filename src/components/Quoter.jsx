  ...
  return (
    <div className="py-12 md:py-24 bg-[#fcfdfe] px-4 md:px-6">
      <div className="max-w-4xl mx-auto text-center mb-10 md:mb-20">
        <h2 className="text-3xl md:text-6xl font-serif italic text-gray-800 mb-4">Cotizá tu Seguro</h2>
        <div className="w-16 h-1 bg-sf-teal mx-auto mb-6"></div>
        <p className="text-gray-500 text-base md:text-lg max-w-xl mx-auto">
          Seleccioná el tipo de cobertura que necesitás y ponete en contacto con nuestros asesores.
        </p>
      </div>

      {/* GRID OPTIMIZADO: 1 columna en móvil, 2 en tablet/desktop */}
      <div className="max-w-5xl mx-auto grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6">
        {insuranceTypes.map((item) => (
          <div 
            key={item.id}
            className={`group bg-white rounded-2xl md:rounded-[2.5rem] border transition-all duration-500 cursor-pointer overflow-hidden ${
              activeId === item.id 
                ? 'border-sf-teal shadow-2xl ring-1 ring-sf-teal/20' 
          ...
          >
            <div className="p-6 md:p-10 flex items-center justify-between gap-4">
              <div className="flex items-center gap-4 md:gap-8">
                {/* ICONO RESPONSIVE: Más chico en móvil */}
                <div className={`transition-all duration-500 ${
                  activeId === item.id ? 'text-sf-teal scale-110' : 'text-gray-300 group-hover:text-gray-400'
                }`}>
                  {React.cloneElement(item.icon, { size: activeId === item.id || window.innerWidth > 768 ? 44 : 32 })}
                </div>
                
                <div>
                  <h3 className="text-lg md:text-2xl font-bold text-gray-800 mb-1 leading-tight">
                    {item.title}
                  </h3>
              ...
              <div className={`text-gray-300 transition-transform duration-500 ${activeId === item.id ? 'rotate-180 text-sf-teal' : ''}`}>
                <ChevronDown size={24} />
              </div>
            </div>

            {/* PANEL DESPLEGABLE CON PADDING AJUSTADO */}
            <div className={`transition-all duration-500 ease-in-out overflow-hidden ${
              activeId === item.id ? 'max-h-[500px] opacity-100 bg-gray-50' : 'max-h-0 opacity-0'
            }`}>
              <div className="p-6 md:p-8 border-t border-gray-100">
                <p className="text-sm md:text-base text-gray-600 mb-6 md:mb-8 leading-relaxed italic">
                  "{item.details}"
                </p>
                ...
                <button 
                  ...
                  className="w-full bg-[#0095ff] hover:bg-[#007acc] text-white py-4 rounded-xl md:rounded-2xl font-bold text-xs tracking-widest flex items-center justify-center gap-3 transition-all shadow-lg active:scale-95 uppercase"
                >
      ...
      <div className="max-w-6xl mx-auto mt-16 text-center">
        <p className="text-gray-400 text-[10px] uppercase tracking-[0.2em] mb-4">Organización Storti Faggiano</p>
      </div>
    </div>
  );
};
